...
var TestLib = /** @class */ (function () {
    function TestLib() {
        this.singleGuardianKey = ["beFA429d57cD18b7F8A4d91A2da9AB4AF05d0FBe"];
        this.singleGuardianPrivKey = [
    ...
    TestLib.prototype.hexStringToUint8Array = function (hs) {
        if (hs.length % 2 === 1) {
            // prepend a 0
            hs = "0" + hs;
        }
        var buf = Buffer.from(hs, "hex");
        var retval = Uint8Array.from(buf);
        return retval;
    };
    TestLib.prototype.uint8ArrayToHexString = function (arr, add0x) {
        var ret = Buffer.from(arr).toString("hex");
        if (!add0x) {
            return ret;
        }
        return "0x" + ret;
    };
    TestLib.prototype.encoder = function (type, val) {
        if (type == "uint8")
    ...
    TestLib.prototype.ord = function (c) {
        return c.charCodeAt(0);
    };
    TestLib.prototype.genGuardianSetUpgrade = function (signers, guardianSet, targetSet, nonce, seq, guardianKeys) {
        var b = [
            "0x",
        ...
        guardianKeys.forEach(function (x) {
            b.push(x);
        });
        var emitter = "0x" + this.zeroBytes.slice(0, 31 * 2) + "04";
    ...
    TestLib.prototype.genGSetFee = function (signers, guardianSet, nonce, seq, amt) {
        var b = [
            "0x",
            this.zeroBytes.slice(0, 28 * 2),
            this.encoder("uint8", this.ord("C")),
    ...
    TestLib.prototype.genGFeePayout = function (signers, guardianSet, nonce, seq, amt, dest) {
        var b = [
            "0x",
            this.zeroBytes.slice(0, 28 * 2),
            this.encoder("uint8", this.ord("C")),
    ...
    TestLib.prototype.genRegisterChain = function (signers, guardianSet, nonce, seq, chain) {
        var b = [
            "0x",
    ...
    TestLib.prototype.genAssetMeta = function (signers, guardianSet, nonce, seq, tokenAddress, chain, decimals, symbol, name) {
        var b = [
            "0x",
            this.encoder("uint8", 2),
        ...
        var emitter = "0x" + this.getTokenEmitter(chain);
        var seconds = Math.floor(new Date().getTime() / 1000.0);
        return this.createSignedVAA(guardianSet, signers, seconds, nonce, chain, emitter, seq, 32, b.join(""));
    };
    TestLib.prototype.genTransfer = function (signers, guardianSet, nonce, seq, amount, tokenAddress, tokenChain, toAddress, toChain, fee) {
        var b = [
            "0x",
            this.encoder("uint8", 1),
        ...
        var emitter = "0x" + this.getTokenEmitter(tokenChain);
        var seconds = Math.floor(new Date().getTime() / 1000.0);
        return this.createSignedVAA(guardianSet, signers, seconds, nonce, tokenChain, emitter, seq, 32, b.join(""));
    };
    /**
     ...
     */
    TestLib.prototype.createSignedVAA = function (guardianSetIndex, signers, timestamp, nonce, emitterChainId, emitterAddress, sequence, consistencyLevel, payload) {
        var body = [
        ...
        var hash = web3Utils.keccak256(web3Utils.keccak256("0x" + body.join("")));
        var signatures = "";
        for (var i in signers) {
            // eslint-disable-next-line new-cap
            var ec = new elliptic.ec("secp256k1");
            var key = ec.keyFromPrivate(signers[i]);
            var signature = key.sign(hash.substr(2), { canonical: true });
            var packSig = [
    ...
    TestLib.prototype.zeroPadBytes = function (value, length) {
