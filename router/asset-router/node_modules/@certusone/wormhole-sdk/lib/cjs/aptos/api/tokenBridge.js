...
// Attest token
var attestToken = function (tokenBridgeAddress, tokenChain, tokenAddress) {
    if (!tokenBridgeAddress)
        throw new Error("Need token bridge address.");
    var assetType = utils_1.getAssetFullyQualifiedType(tokenBridgeAddress, utils_1.coalesceChainId(tokenChain), tokenAddress);
    if (!assetType)
...
// Complete transfer
var completeTransfer = function (client, tokenBridgeAddress, transferVAA, feeRecipient) { return __awaiter(void 0, void 0, void 0, function () {
    ...
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                if (!tokenBridgeAddress)
                    throw new Error("Need token bridge address.");
                parsedVAA = algorand_1._parseVAAAlgorand(transferVAA);
                ...
                if (!(parsedVAA.FromChain === utils_1.CHAIN_ID_APTOS)) return [3 /*break*/, 2];
                return [4 /*yield*/, utils_1.getTypeFromExternalAddress(client, tokenBridgeAddress, parsedVAA.Contract)];
            case 1:
                _a = _b.sent();
                return [3 /*break*/, 3];
            case 2:
                _a = utils_1.getAssetFullyQualifiedType(tokenBridgeAddress, utils_1.coalesceChainId(parsedVAA.FromChain), parsedVAA.Contract);
                _b.label = 3;
            case 3:
                assetType = _a;
                if (!assetType)
...
var completeTransferAndRegister = function (client, tokenBridgeAddress, transferVAA) { return __awaiter(void 0, void 0, void 0, function () {
    var parsedVAA, assetType, _a;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
...
var completeTransferWithPayload = function (_tokenBridgeAddress, _tokenChain, _tokenAddress, _vaa) {
 ...
 */
var registerCoin = function (tokenBridgeAddress, originChain, originAddress) {
    var bytecode = utils_1.hexToUint8Array("");
    var assetType = utils_1.getAssetFullyQualifiedType(tokenBridgeAddress, utils_1.coalesceChainId(originChain), originAddress);
    if (!assetType)
        throw new Error("Asset type is null");
    var typeTag = new aptos_1.TxnBuilderTypes.TypeTagStruct(aptos_1.TxnBuilderTypes.StructTag.fromString(assetType));
...
var deployCoin = function (tokenBridgeAddress) {
    if (!tokenBridgeAddress)
        throw new Error("Need token bridge address.");
    return {
...
// Register chain
var registerChain = function (tokenBridgeAddress, vaa) {
    if (!tokenBridgeAddress)
        throw new Error("Need token bridge address.");
    return {
...
// Transfer tokens
var transferTokens = function (tokenBridgeAddress, fullyQualifiedType, amount, recipientChain, recipient, relayerFee, nonce) {
    if (!tokenBridgeAddress)
        throw new Error("Need token bridge address.");
...
var transferTokensWithPayload = function (tokenBridgeAddress, fullyQualifiedType, amount, recipientChain, recipient, relayerFee, nonce, payload) {
...
var createWrappedCoinType = function (tokenBridgeAddress, vaa) {
    if (!tokenBridgeAddress)
        throw new Error("Need token bridge address.");
    return {
...
var createWrappedCoin = function (tokenBridgeAddress, attestVAA) {
    if (!tokenBridgeAddress)
        throw new Error("Need token bridge address.");
    var parsedVAA = algorand_1._parseVAAAlgorand(attestVAA);
    if (!parsedVAA.FromChain || !parsedVAA.Contract) {
        throw new Error("VAA does not contain required information");
    }
    utils_1.assertChain(parsedVAA.FromChain);
    var assetType = utils_1.getAssetFullyQualifiedType(tokenBridgeAddress, utils_1.coalesceChainId(parsedVAA.FromChain), parsedVAA.Contract);
    if (!assetType)
        throw new Error("Invalid asset address.");
    return {
