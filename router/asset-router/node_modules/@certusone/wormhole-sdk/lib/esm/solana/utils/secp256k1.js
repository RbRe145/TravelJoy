...
export var SIGNATURE_LENGTH = 65;
export var ETHEREUM_KEY_LENGTH = 20;
/**
 ...
 */
var Secp256k1SignatureOffsets = /** @class */ (function () {
    function Secp256k1SignatureOffsets() {
    }
     ...
     */
    Secp256k1SignatureOffsets.serialize = function (signatures, keys, message) {
        ...
        var numSignatures = signatures.length;
        var offsetSpan = 11;
        var dataLoc = 1 + numSignatures * offsetSpan;
        var dataLen = SIGNATURE_LENGTH + ETHEREUM_KEY_LENGTH; // 65 signature size + 20 eth pubkey size
        var messageDataOffset = dataLoc + numSignatures * dataLen;
        var messageDataSize = 32;
        var serialized = Buffer.alloc(messageDataOffset + messageDataSize);
        ...
        for (var i = 0; i < numSignatures; ++i) {
            var signature = signatures.at(i);
            if ((signature === null || signature === void 0 ? void 0 : signature.length) != SIGNATURE_LENGTH) {
                throw Error("signatures[" + i + "].length != 65");
            }
            var key = keys.at(i);
            if ((key === null || key === void 0 ? void 0 : key.length) != ETHEREUM_KEY_LENGTH) {
                throw Error("keys[" + i + "].length != 20");
            }
            var signatureOffset = dataLoc + dataLen * i;
            var ethAddressOffset = signatureOffset + 65;
    ...
    return Secp256k1SignatureOffsets;
}());
export { Secp256k1SignatureOffsets };
