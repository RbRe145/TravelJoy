...
function deriveWormholeBridgeDataKey(wormholeProgramId) {
    return utils_1.deriveAddress([Buffer.from("Bridge")], wormholeProgramId);
}
    ...
    return __awaiter(this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            return [2 /*return*/, connection
                    ...
                    .then(function (info) { return BridgeData.deserialize(utils_1.getAccountData(info)); })];
        });
    });
}
exports.getWormholeBridgeData = getWormholeBridgeData;
var BridgeConfig = /** @class */ (function () {
    function BridgeConfig(guardianSetExpirationTime, fee) {
        this.guardianSetExpirationTime = guardianSetExpirationTime;
        this.fee = fee;
    }
    BridgeConfig.deserialize = function (data) {
        if (data.length != 12) {
            throw new Error("data.length != 12");
        }
        var guardianSetExpirationTime = data.readUInt32LE(0);
        var fee = data.readBigUInt64LE(4);
        return new BridgeConfig(guardianSetExpirationTime, fee);
    };
    return BridgeConfig;
}());
exports.BridgeConfig = BridgeConfig;
var BridgeData = /** @class */ (function () {
    function BridgeData(guardianSetIndex, lastLamports, config) {
        this.guardianSetIndex = guardianSetIndex;
        this.lastLamports = lastLamports;
        this.config = config;
    }
    BridgeData.deserialize = function (data) {
        if (data.length != 24) {
            throw new Error("data.length != 24");
        }
        var guardianSetIndex = data.readUInt32LE(0);
        var lastLamports = data.readBigUInt64LE(4);
        var config = BridgeConfig.deserialize(data.subarray(12));
